...
*/
	...
	int* intSendPack = new int[16400]();// multiplier 3 for 3 different elements of index, newModules, oldModules and +1 for elementCount
	int* intReceivePack = new int[16400]();

	for (int prId = 0; prId < size; prId++) {
		if (prId != rank) {
			printf("testing message send from sender:%d and receiver:%d\n",
					rank, prId);
			MPI_Send(intSendPack, 16400, MPI_INT, prId, 0,
			MPI_COMM_WORLD);
		  }
	}
	...
	for (int sId = 0; sId < size; sId++) {
		if (sId != rank) {
			MPI_Recv(intReceivePack, 16400, MPI_INT, sId, 0,
