...
Node::Node() :
		id(0), size(0.0), exitPr(0.0), nodeWeight(1.0), modIdx(0), isModule(
				false), isSuper(false), orig_module(NULL), teleportWeight(0.0), danglingSize(
				0.0), isDangling(false) {
}

Node::Node(int id) :
		id(id), size(0.0), exitPr(0.0), nodeWeight(1.0), modIdx(0), isModule(
				false), isSuper(false), orig_module(NULL), teleportWeight(0.0), danglingSize(
				0.0), isDangling(false) {
}

Node::Node(int id, double size) :
		id(id), size(size), exitPr(0.0), nodeWeight(1.0), modIdx(0), isModule(
				false), isSuper(false), orig_module(NULL), teleportWeight(0.0), danglingSize(
				0.0), isDangling(false) {
}

Node::Node(int id, double size, int module, bool isModule,
		Module *original_module) :
		id(id), size(size), exitPr(0.0), nodeWeight(1.0), modIdx(module), isModule(
				isModule), isSuper(false), orig_module(original_module), teleportWeight(
				0.0), danglingSize(0.0), isDangling(false) {
}
...
SuperNode::SuperNode(Module module, int newID) {
	...
	if (numMembers != module.numMembers) {
		printf("SuperNode[%d],members=%d,moduleMembers:%d\n", id, numMembers,
				module.numMembers);
		cout << "SuperNode[" << id << "] members.size() != numMembers" << endl;
	}
	if (members.size() != module.members.size())
		cout << "SuperNode[" << id
				<< "] members.size() != module.members.size()" << endl;
}
